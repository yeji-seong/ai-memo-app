...
메모 CRUD 기능을 제공하는 Next.js 웹 애플리케이션. Supabase PostgreSQL을 사용하여 데이터를 영속화하며, 카테고리 분류 및 검색 기능을 지원한다.
...
2. 데이터베이스 접근은 반드시 서버 액션을 통해서만 수행
3. 메모 ID 생성은 uuid v4만 사용 (DB는 gen_random_uuid() 사용)
4. TypeScript strict mode 위반 금지
5. Supabase 클라이언트 직접 import 금지 (서버 액션 사용)
...
- LocalStorage 사용 금지 (데이터는 Supabase에 저장)
...
src/
├── app/
│   ├── actions/        # 서버 액션 (DB 접근 계층)
│   ├── api/            # API 라우트 (Gemini 요약)
│   ├── layout.tsx      # 루트 레이아웃
│   └── page.tsx        # 메인 페이지
...
└── utils/              # 유틸리티 함수 (deprecated: localStorage, seedData)
```
...
- **[서버 액션 (Actions)](./src/app/actions/)** — 데이터베이스 접근 및 서버 측 로직 작업 시
